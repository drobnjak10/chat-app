...
const { generateMsg, generateLocationMessage } = require('./utils/messages');
...
io.on('connection', (socket) => {
    ...
    socket.on('join', ( options, callback ) => {
        ...
        socket.broadcast.to(user.room).emit('message', generateMsg(user.username,`${user.username} has joined the chat!`));
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            io.to(user.room).emit('message', generateMsg(`A ${user.username} has left the chat.`))
